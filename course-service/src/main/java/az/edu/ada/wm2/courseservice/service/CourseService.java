...
public class CourseService {
    ...
    public EnrollmentResponseDto enrollStudent(Long courseId, Long studentId) {
        ...
        Long prereq = course.getPrerequisiteCourseId();
        if (prereq != null) {
            boolean hasPrereq = enrollmentRepository.existsByCourseIdAndStudentId(prereq, studentId);
            if (!hasPrereq) {
                throw new PrerequisiteNotMetException(courseId, prereq, studentId);
            }
        }
        ...
        return new EnrollmentResponseDto(
                ...
                savedEnrollment.getEnrollmentDate().format(ISO),
    ...
    public List<CourseResponseDto> getCoursesByStudentName(String name) {
        // call student-service to find matching students
        List<StudentDto> matches = studentFeignClient.searchStudentsByName(name);
        List<Long> studentIds = matches.stream().map(StudentDto::getId).toList();

        List<Long> courseIds = studentIds.stream()
                .flatMap(id -> enrollmentRepository.findByStudentId(id).stream())
